...
export default function HeroPoints({ totalPoints, progress, targetPoints }) {
    return (
            ...
            <div className="relative z-10">
                <div className="flex items-center justify-between mb-6 sm:mb-8">
                    <div className="flex items-center gap-2 sm:gap-3">
                        ...
                        <div>
                            <span className="text-white/80 text-xs sm:text-sm font-medium">Total Poin</span>
                ...
                <div className="mb-6 sm:mb-8">
                    ...
                    <p className="text-white/70 text-sm sm:text-base mt-2">
                        Total point didapatkan {targetPoints.toLocaleString('id-ID')} poin
                    </p>
                </div>

                <div className="space-y-3 sm:space-y-4">
                    <div className="flex justify-between items-center text-white">
                        <span className="text-sm sm:text-base font-medium">Progress ke Hadiah</span>
                        <span className="text-lg sm:text-xl font-bold">{Math.round(progress)}%</span>
                    </div>
                    
                    <div className="relative">
                        <div className="h-3 sm:h-4 bg-white/20 backdrop-blur-sm rounded-full overflow-hidden">
                            <div className="h-full bg-white rounded-full transition-all duration-700 ease-out shadow-lg" style={{ width: `${Math.min(progress, 100)}%` }}>
                                <div className="w-full h-full bg-gradient-to-r from-white to-white/90 animate-pulse"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <p className="text-white text-sm sm:text-base font-medium">
                        {targetPoints - totalPoints > 0
                            ? `${(targetPoints - totalPoints).toLocaleString('id-ID')} poin lagi untuk hadiah`
                            : "Siap ditukar! 🎉"}
                    </p>
                </div>
            </div>
